import { KiviPlugin, segment, makeForwardMsg } from '@kivibot/core'
    ...
    plugin.onMatch(cmd, async e => {
      ...
      if (platform === 'bili') {
        ...
        if (!share) {
          ...
          e.reply(segment.share(url, title, image, content))
        }
      } else {
        const id = await fetchSongId(songName, platform)
        ...
        if (e.message_type === 'group') {
          if (platform === 'migu') {
            shareMiGuSong.bind(bot)(id, e.group_id, true)
          } else if (platform === 'kuwo') {
            shareKuwoSong.bind(bot)(id, e.group_id, true)
          } else {
            e.group.shareMusic(platform, id)
          }
        } else if (e.message_type === 'private') {
          if (platform === 'migu') {
            shareMiGuSong.bind(bot)(id, e.friend.uid, false)
          } else if (platform === 'kuwo') {
            shareKuwoSong.bind(bot)(id, e.friend.uid, false)
          } else {
