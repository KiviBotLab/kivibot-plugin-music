import { core, http } from '@kivibot/core'
import { decode } from 'html-entities'

import type { Client } from '@kivibot/core'

export type Platform = 'qq' | '163' | 'migu' | 'kugou' | 'kuwo'

export async function fetchSongId(songName: string, platform: Platform) {
  if (platform === 'qq') return fetchQQId(songName)
  if (platform === '163') return fetch163Id(songName)
  if (platform === 'migu') return fetchMiguId(songName)
  if (platform === 'kugou') return fetchKugouId(songName)
  if (platform === 'kuwo') return fetchKuwoId(songName)
}

export async function fetchBiliShare(songName: string) {
...
export async function fetchLyric(songName: string) {
  ...
  const { data } = await http.get(api, { params, headers: { Referer: 'https://y.qq.com' } })
  const ls = (data?.lyric ?? '').replace(/\[(.*?)]/g, '').replace(/\n(\n)*/g, '\n')

  return decode(ls.trim())
}

async function fetchQQId(songName: string, isMid = false) {
...
async function fetch163Id(songName: string) {
  const api = 'https://music.163.com/api/cloudsearch/pc'
  const params = { s: songName, type: 1, offset: 0 }

  const { data } = await http.get(api, { params, headers: { 'Accept-Encoding': '*' } })

  return data?.result?.songs?.[0]?.id ?? ''
}

async function fetchMiguId(songName: string) {
  const api = 'https://m.music.migu.cn/migu/remoting/scr_search_tag'
  const params = { rows: 1, type: 2, keyword: songName, pgc: 1 }

  const { data } = await http.get(api, { params, headers: { Referer: 'https://m.music.migu.cn' } })

  return data?.musics?.[0]?.copyrightId ?? ''
}

async function fetchKugouId(songName: string) {
...
async function fetchKuwoId(songName: string) {
...
export async function shareKuwoSong(this: Client, id: string, target: number, isGroup = true) {
  let rsp: any = await http.get(
    `http://yinyue.kuwo.cn/api/www/music/musicInfo?mid=${id}&httpsStatus=1`,
    { responseType: 'json', headers: { csrf: id, cookie: ' kw_token=' + id } }
  )
  rsp = rsp.data.data
  const url: any = await http.get(
  ...
  await this.sendOidb('OidbSvc.0xb77_9', core.pb.encode(body))
}
...
export async function shareMiGuSong(this: Client, id: string, target: number, isGroup = true) {
  let rsp: any = await http.get(
